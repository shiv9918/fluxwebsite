...
const HODHero: React.FC<HODHeroProps> = React.memo(({ hod }) => {
  ...
  return (
    <motion.section
      className="relative overflow-hidden bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800 rounded-2xl shadow-lg mb-12"
      initial="hidden"
      animate="visible"
      variants={containerVariants}
    >
      <div className="absolute inset-0 bg-grid-gray-200/20 [mask-image:linear-gradient(0deg,transparent,white)] dark:[mask-image:linear-gradient(0deg,transparent,rgba(0,0,0,0.5))]" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <motion.div className="space-y-6 lg:space-y-8">
            ...
            >
              ...
              <span className="text-lg font-medium text-gray-600 dark:text-gray-300">{hod.department}</span>
            </motion.div>
            ...
            <motion.div 
              ...
              className="pt-4"
            >
              <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-3">Areas of Expertise</h3>
              <div className="flex flex-wrap gap-2">
                {hod.specialties.map((specialty, index) => (
                  <motion.span
                    key={specialty}
                    className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-white/80 dark:bg-gray-800/80 text-gray-800 dark:text-gray-200 shadow-sm"
            ...
            >
              {socialIcons.map(({ icon: Icon, href, label }, index) => (
                <motion.a
                  ...
                  className="flex items-center justify-center w-10 h-10 bg-white/80 hover:bg-blue-50 dark:bg-gray-700/80 dark:hover:bg-blue-900/30 rounded-full shadow-md hover:shadow-lg transition-all duration-300"
                  ...
                  aria-label={label}
                >
                  <Icon className="w-5 h-5 text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200" />
                </motion.a>
              ))}
            </motion.div>
          </motion.div>
          ...
          >
            <motion.div
              className="relative w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 rounded-2xl overflow-hidden shadow-2xl border-4 border-white dark:border-gray-800"
